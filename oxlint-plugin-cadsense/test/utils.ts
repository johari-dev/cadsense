...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-cadsense", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "cadsense", specifier: pluginPath }],
