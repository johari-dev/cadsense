...
import { fromYaml } from "@cadsense/shared/schemaYaml";
import { HostProcessArchitecture, HostProcessPlatform } from "@cadsense/shared/hostProcess";
...
import {
  ...
  selectServerRuntimeExternalDependencies,
} from "./lib/server-bundle-externals.ts";
...
const DESKTOP_APP_ID = "com.cadsense.cadsense";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "cadsense-resource-monitor.exe" : "cadsense-resource-monitor";
}
...
export class InlinedNativePackageError extends Schema.TaggedErrorClass<InlinedNativePackageError>()(
  ...
  override get message(): string {
    return `The server bundle inlined packages that load native binaries: ${this.packages.join(", ")}. A node-gyp-build style loader resolves prebuilds relative to its own file, so inlined into a chunk it finds none and the importer quietly falls back to a slower JS path. Add them to SERVER_RUNTIME_EXTERNAL_PREFIXES in scripts/lib/server-bundle-externals.ts so they stay external and are staged in the sidecar.`;
  }
}
...
interface StagePackageJson {
  ...
  readonly cadsenseCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Cadsense always passes the user's installed Claude executable to the SDK,
...
export function resolveMacStageDependencies(input: {
  ...
  return {
    ...selectServerRuntimeExternalDependencies(input.serverDependencies),
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "CADSENSE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("CADSENSE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "CADSENSE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("CADSENSE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("CADSENSE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("CADSENSE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("CADSENSE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("CADSENSE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("CADSENSE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("CADSENSE_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("CADSENSE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(
    Config.option,
  ),
  ...
  wslPrebuild: Config.string("CADSENSE_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
const verifyPackagedBundleIsSelfContained = Effect.fn("verifyPackagedBundleIsSelfContained")(
  function* (input: { readonly asarPath: string; readonly verbose: boolean }) {
    ...
    const probeRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "cadsense-bundle-selfcheck-",
    });
...
export const stageResourceMonitor = Effect.fn("stageResourceMonitor")(function* (input: {
  ...
  const reuseResourceMonitor = yield* Config.boolean(
    "CADSENSE_DESKTOP_REUSE_RESOURCE_MONITOR",
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("CADSENSE_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
// node-pty's prebuilds/linux-<arch>/ with a cadsense marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / CADSENSE_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(
    path.join(prebuildDir, "cadsense-wsl-node-pty.json"),
    `${markerJson}\n`,
  );
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "cadsense-windows-primary-native-probe-",
  });
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  if (!(yield* isFile(resourceMonitorPath))) {
    return yield* new WindowsPackagedPayloadValidationError({
      ...
      packagedAppDir,
      missingFiles: ["resource-monitor/cadsense-resource-monitor.exe"],
    });
  }
    ...
    const requiredMembers = [
      ...
      ...(wslArch === undefined
        ...
        : [
            ...
            `node_modules/node-pty/prebuilds/linux-${wslArch}/cadsense-wsl-node-pty.json`,
          ]),
    ];
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const resolvedServerRuntimeExternalDependencies = selectServerRuntimeExternalDependencies(
  ...
  const stageRoot = yield* mkdir({
    prefix: `cadsense-desktop-${options.platform}-stage-`,
  });
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @cadsense/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: CADSENSE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: CADSENSE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription(
      "Build arch, for example arm64/x64/universal (env: CADSENSE_DESKTOP_ARCH).",
    ),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: CADSENSE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: CADSENSE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: CADSENSE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: CADSENSE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: CADSENSE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: CADSENSE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: CADSENSE_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription(
      "Mock update server port (env: CADSENSE_DESKTOP_MOCK_UPDATE_SERVER_PORT).",
    ),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: CADSENSE_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
