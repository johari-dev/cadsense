...
const makeWindowsPayloadFixture = Effect.fn("test.makeWindowsPayloadFixture")(function* (input: {
  ...
  const tempDir = yield* fs.makeTempDirectoryScoped({
    prefix: "cadsense-windows-payload-test-",
  });
  ...
  yield* fs.writeFileString(
    path.join(resourcesDir, "resource-monitor/cadsense-resource-monitor.exe"),
    "monitor",
  );
  const appExecutableName = "cadsense.exe";
  ...
  if (input.wslRuntime !== undefined) {
    ...
    yield* fs.writeFileString(
      path.join(linuxPrebuildDir, "cadsense-wsl-node-pty.json"),
      '{"arch":"x64"}',
    );
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                CADSENSE_DESKTOP_UPDATE_REPOSITORY: "johari-dev/cadsense",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "johari-dev/cadsense",
              },
            }),
          ),
        ),
  ...
  it.effect("omits update feeds for pull request preview builds", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromEnv({ env: { GITHUB_REPOSITORY: "johari-dev/cadsense" } }),
        ),
      ),
    ),
  );
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@cadsense/client-runtime": "workspace:*",
          "@cadsense/contracts": "workspace:*",
          "@cadsense/shared": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
    ...
    () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const tempDir = yield* fs.makeTempDirectoryScoped({
            prefix: "cadsense-windows-architecture-test-",
          });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const repoRoot = yield* fs.makeTempDirectoryScoped({
          prefix: "cadsense-resource-monitor-cache-test-",
        });
        const binaryPath = path.join(
          repoRoot,
          "native/resource-monitor/target/x86_64-unknown-linux-gnu/release/cadsense-resource-monitor",
        );
        ...
        yield* stageResourceMonitor({
          ...
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { CADSENSE_DESKTOP_REUSE_RESOURCE_MONITOR: "true" },
              }),
            ),
          ),
        );
        ...
        assert.equal(
          yield* fs.readFileString(
            path.join(stageResourcesDir, "resource-monitor/cadsense-resource-monitor"),
          ),
          "cached monitor",
        );
      }),
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const resourceMonitorPath = path.join(
          fixture.packagedAppDir,
          "resources/resource-monitor/cadsense-resource-monitor.exe",
        );
        ...
        assert.deepStrictEqual(resourceMonitorError.missingFiles, [
          "resource-monitor/cadsense-resource-monitor.exe",
        ]);
      }),
    ),
  );
  ...
  it.effect("rejects a sidecar whose extracted server bundle cannot resolve", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fixture = yield* makeWindowsPayloadFixture({
          copyUnpackedNatives: true,
          serverEntrySource: 'import "cadsense-deliberately-missing-package";\n',
        });
        const error = yield* validateWindowsPackagedPayload({
          stageDistDir: fixture.stageDistDir,
          appExecutableName: fixture.appExecutableName,
        ...
        assert.include(error.output, "cadsense-deliberately-missing-package");
      }),
    ),
  );
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "cadsense-dmg-background-",
        });
  ...
  it.effect("fails clearly when the selected DMG background source is missing", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "cadsense-dmg-background-missing-",
        });
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "cadsense-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "cadsense-resource-monitor.exe");
  });
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const stageRoot = yield* fs.makeTempDirectoryScoped({
          prefix: "cadsense-wsl-runtime-archive-",
        });
  ...
  it.effect("ships only Linux runtime members in the WSL archive", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-wsl-runtime-members-" });
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                CADSENSE_DESKTOP_SKIP_BUILD: "true",
                CADSENSE_DESKTOP_KEEP_STAGE: "true",
                CADSENSE_DESKTOP_SIGNED: "true",
                CADSENSE_DESKTOP_VERBOSE: "true",
                CADSENSE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
...
it.effect("rebases packaged links into the isolated tree", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const root = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-copy-symlinks-" });
