// @effect-diagnostics nodeBuiltinImport:off - builds temporary workspace layouts on disk.
...
} from "@cadsense/shared/hostProcess";
...
const devServerInput = {
  mode: "dev:server",
  cadsenseHome: "/tmp/cadsense-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit CADSENSE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "CADSENSE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "CADSENSE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the default home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_HOME, undefined);
        assert.equal(env.CADSENSE_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          cadsenseHome: "/tmp/custom-cadsense",
          browser: false,
        ...
        assert.equal(env.CADSENSE_HOME, path.resolve("/tmp/custom-cadsense"));
        assert.equal(env.CADSENSE_PORT, "4222");
        assert.equal(env.CADSENSE_NO_BROWSER, "1");
        assert.equal(env.CADSENSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.CADSENSE_LOG_WS_EVENTS, "1");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CADSENSE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          cadsenseHome: undefined,
        ...
        assert.equal(env.CADSENSE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom cadsenseHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.CADSENSE_HOME, path.resolve("/tmp/my-cadsense"));
      }),
    );

    it.effect("pins desktop dev to stable backend and renderer ports", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
        ...
        assert.equal(env.CADSENSE_PORT, "4222");
        assert.equal(env.CADSENSE_NO_BROWSER, undefined);
      }),
    );
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CADSENSE_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    // interface while the page itself still loads.
  ...
  describe("findFirstAvailableOffset", () => {
    ...
    it.effect("skips browser-blocked web ports before probing availability", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(probed, [14_607, 6567]);
      }),
    );
  ...
  describe("isBrowserAllowedPort", () => {
    it.each([6000, 6566, 6665, 6666, 6667, 6668, 6669, 6679, 6697])(
      "rejects Fetch-blocked web port %s from the workspace offset range",
      (port) => {
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { CADSENSE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["CADSENSE_PORT_OFFSET", "CADSENSE_DEV_INSTANCE"]);
    ...
    // The env builder must not consult ambient CADSENSE_HOME on its own; the
    // runner always supplies an explicit workspace-local directory.
    it.effect("ignores an ambient CADSENSE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { CADSENSE_HOME: "/home/user/.cadsense" },
          serverOffset: 0,
          webOffset: 0,
          cadsenseHome: undefined,
          browser: undefined,
    ...
    describe("cadsense home precedence", () => {
      const makeWorkspace = Effect.acquireRelease(
        Effect.sync(() =>
          NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "cadsense-devrunner-")),
        ),
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({
            ...devServerInput,
            cadsenseHome: input.cadsenseHome,
          }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            ...
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { CADSENSE_HOME: input.ambientHome },
            ),
          );

          return captured?.CADSENSE_HOME;
        });

      it.effect("prefers an explicit --home-dir over the workspace default", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorkspace;
          const home = yield* spawnedHome({
            cadsenseHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.cadsense",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".cadsense"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the workspace .cadsense over an ambient CADSENSE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorkspace;
          const home = yield* spawnedHome({
