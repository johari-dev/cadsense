...
describe("shouldBundleServerDependency", () => {
  it("bundles ordinary runtime dependencies", () => {
    for (const id of ["effect", "@effect/platform", "hono", "@cadsense/shared/hostProcess"]) {
      assert.strictEqual(shouldBundleServerDependency(id), true, id);
    }
  });

  it("never bundles node: builtins", () => {
    assert.strictEqual(shouldBundleServerDependency("node:fs"), false);
  });
  ...
  it("leaves native addons and their dlopen wrappers external", () => {
    for (const id of [
      ...
      assert.strictEqual(shouldBundleServerDependency(id), false, id);
    }
  });

  it("leaves bun-only entry points external", () => {
    assert.strictEqual(shouldBundleServerDependency("@effect/platform-bun"), false);
    assert.strictEqual(shouldBundleServerDependency("@effect/sql-sqlite-bun"), false);
  });
  ...
  it("treats prefix-matched siblings as external", () => {
    assert.strictEqual(shouldBundleServerDependency("node-gyp-build-optional-packages"), false);
  });
});

describe("selectServerRuntimeExternalDependencies", () => {
  ...
  it("selects every external root declared by the server", () => {
    assert.deepStrictEqual(
      ...
      ["@ff-labs/fff-node", "msgpackr-extract"],
    );
  });
});
...
it.layer(NodeServices.layer)("external package dependency closure", (it) => {
  ...
  const isRuntimeExternal = (name: string) =>
    SERVER_RUNTIME_EXTERNAL_PREFIXES.some((prefix) => name.startsWith(prefix));
  ...
  it.effect("finds the runtime-external packages on disk", () =>
    Effect.gen(function* () {
      ...
      for (const required of ["node-gyp-build-optional-packages", "detect-libc"]) {
        assert.ok(
