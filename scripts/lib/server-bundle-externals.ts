/**
 * The single source of truth for packages the packaged server bundle must NOT inline.
 *
 ...
 */
export const SERVER_RUNTIME_EXTERNAL_PREFIXES = [
  "node-pty",
  "ffi-rs",
  "@yuuang/",
  "@ff-labs/",
...
export const SERVER_EXTERNAL_PACKAGE_PREFIXES = [
  ...
  ...SERVER_BUILD_ONLY_EXTERNAL_PREFIXES,
] as const;

export function isRuntimeExternalServerDependency(id: string): boolean {
  return SERVER_RUNTIME_EXTERNAL_PREFIXES.some((prefix) => id.startsWith(prefix));
}
 ...
 */
export function isExternalServerDependency(id: string): boolean {
  return SERVER_EXTERNAL_PACKAGE_PREFIXES.some((prefix) => id.startsWith(prefix));
}

/** True when the packaged server bundle should inline `id` rather than leave it external. */
export function shouldBundleServerDependency(id: string): boolean {
  if (id.startsWith("node:")) return false;
  return !isExternalServerDependency(id);
}

/** Select direct dependency roots whose runtime closure belongs in the sidecar. */
export function selectServerRuntimeExternalDependencies(
  dependencies: Readonly<Record<string, string>>,
): Record<string, string> {
  return Object.fromEntries(
    Object.entries(dependencies).filter(([name]) => isRuntimeExternalServerDependency(name)),
  );
}
...
export function findInlinedExternalPackages(source: string): {
  ...
  for (const region of source.matchAll(regionPattern)) {
    ...
    for (const candidate of regionPath.matchAll(packagePattern)) {
      ...
      if (isExternalServerDependency(name)) inlined.add(name);
    }
  }
