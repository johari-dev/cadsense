...
repo='johari-dev/cadsense'
tag="${RELEASE_TAG:?RELEASE_TAG is required}"
pkgrel="${PKGREL:-1}"

if [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  pkgname='cadsense-bin'
...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Cadsense-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit cadsense-icon.png
git config user.name 'cadsense-ci'
git config user.email 'cadsense-ci@users.noreply.github.com'
git add -A
