...
describe("collectComposerInlineTokens", () => {
  ...
  it.each(["@scope/ui", "@jane/foo.js", "@scope/pkg/sub/path"])(
  ...
  it("keeps scoped package references plain across incomplete input and IME whitespace", () => {
    expect(collectComposerInlineTokens("Install @scope/ui")).toEqual([]);
    expect(collectComposerInlineTokens("入力 @scope/ui　を追加")).toEqual([]);
  });
  ...
  it("allows ambiguous scoped paths through explicit quoted mentions", () => {
    expect(collectComposerInlineTokens('Inspect @"scope/ui" next')).toEqual([
      {
        type: "mention",
        value: "scope/ui",
        source: '@"scope/ui"',
