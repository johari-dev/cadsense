...
const PATH_CAPTURE_START = "__CADSENSE_PATH_START__";
const PATH_CAPTURE_END = "__CADSENSE_PATH_END__";
...
/**
 * Escapes a single argument for `cmd.exe` shell mode on Windows. We assemble
 * one command line before spawning because Node deprecates combining
 * `shell: true` with a separate argument array. Every dynamic argument still
 * needs to survive cmd.exe and the target program's `CommandLineToArgvW`
...
export const SpawnExecutableResolution = Context.Reference<SpawnExecutableResolver>(
  "@cadsense/shared/shell/SpawnExecutableResolution",
  {
...
function envCaptureStart(name: string): string {
  return `__CADSENSE_ENV_${name}_START__`;
}

function envCaptureEnd(name: string): string {
  return `__CADSENSE_ENV_${name}_END__`;
}
...
export const WindowsShellEnvironment = Context.Reference<WindowsShellEnvironmentReader>(
  "@cadsense/shared/shell/WindowsShellEnvironment",
  {
...
export const CommandAvailability = Context.Reference<CommandAvailabilityChecker>(
  "@cadsense/shared/shell/CommandAvailability",
  {
...
export const CommandResolutionCache = Context.Reference<Map<string, CommandResolutionCacheEntry>>(
  "@cadsense/shared/shell/CommandResolutionCache",
  {
