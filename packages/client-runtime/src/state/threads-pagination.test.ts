...
describe("thread pagination state", () => {
  ...
  it.effect("a merged history page never advances the live-event dedupe sequence", () =>
    Effect.gen(function* () {
      ...
      // Event at sequence 11 must still apply after the merge. If the merge
      // had advanced the dedupe sequence to the page's 12, it would be swallowed.
      yield* Queue.offer(harness.inputs, titleEvent("Event after merge", 11));
      const state = yield* harness.awaitState((value) =>
        Option.match(value.data, {
          onNone: () => false,
          onSome: (thread) => thread.title === "Event after merge",
        }),
      );
