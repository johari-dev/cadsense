...
export class ThreadOlderTurnRequests extends Context.Reference<ThreadOlderTurnRequestRegistry>(
  "@cadsense/client-runtime/state/threads/ThreadOlderTurnRequests",
...
export const makeEnvironmentThreadState = Effect.fn("EnvironmentThreadState.make")(function* (
  ...
  // in-flight older-page fetch (snapshot replacement or deletion). A
  ...
  // merges. Without it, a snapshot processed between loadOlderTurns'
  ...
  const setDisconnected = Effect.gen(function* () {
    ...
    // window parameters to a server that may not accept them (compatibility
  ...
  const loadOlderTurns = Effect.fn("EnvironmentThreadState.loadOlderTurns")(function* () {
    ...
    // application, so a snapshot cannot land between them (TOCTOU
    // guard) — anything that rewrites history bumps the epoch
    ...
    yield* applyLock.withPermits(1)(
      Effect.gen(function* () {
        ...
        // resurrect turns a newer snapshot already removed.
        const stale =
