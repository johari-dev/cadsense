...
describe("getProviderSkillsForSlashMenu", () => {
  it("keeps the skill alias when the provider also exposes it as a slash command", () => {
    ...
    expect(getProviderSkillsForSlashMenu([askMatt]).map((skill) => skill.name)).toEqual([
      "ask-matt",
    ]);
  });
});
...
describe("getProviderSkillsForSlashMenu", () => {
  it("drops a skill the provider reserves for the agent", () => {
    ...
    expect(getProviderSkillsForSlashMenu(skills).map((skill) => skill.name)).toEqual(["deploy"]);
  });
});
...
describe("resolveProviderSkillSourceKind", () => {
  it("marks plugin-backed skills as app installs", () => {
    expect(
      resolveProviderSkillSourceKind({
        path: "/Users/julius/.codex/plugins/cache/openai-curated/tools/skills/fix-ci/SKILL.md",
        scope: "user",
      }),
    ).toBe("app");
  });
