...
>()("@cadsense/client-runtime/connection/supervisor/EnvironmentSupervisor") {}
...
> {
  ...
  const traceConnectionEstablishment = (
    ...
    }).pipe(Effect.withSpan("environment.connection.attempt", { root: true }));
  ...
  const establishTracedConnection = Effect.fnUntraced(function* (
    ...
    return yield* traceConnectionEstablishment(
