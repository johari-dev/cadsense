...
describe("connection presentation", () => {
  it("preserves profile display information without exposing credentials", () => {
    expect(connectionCatalogDisplayUrl(ENTRY)).toBe("http://127.0.0.1:3774");
  });
    ...
    expect(
      presentEnvironmentConnection(
        supervisorState({
          ...
          lastFailure: new ConnectionTransientError({
            reason: "transport",
            detail: "Local connection timed out.",
            traceId: "trace-retry",
          }),
        }),
      ),
      ...
      phase: "reconnecting",
      error: "Local connection timed out.",
      traceId: "trace-retry",
    });
  });
  ...
  it("combines reconnect progress with the latest failure", () => {
    ...
    expect(connectionStatusText(connection)).toBe(
      "Failed to connect. Reconnecting... Reason: Local request timed out.",
    );
