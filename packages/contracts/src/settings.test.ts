...
describe("ClientSettings thread actions", () => {
...
describe("ServerSettings assistant text streaming", () => {
  it("streams by default while allowing an explicit opt-out", () => {
    expect(decodeServerSettings({}).streamAssistantText).toBe(true);
    expect(decodeServerSettingsPatch({ streamAssistantText: false }).streamAssistantText).toBe(
      false,
    );
  });
});
...
describe("provider enabled defaults", () => {
  ...
  it("derives per-driver defaults from the settings schemas", () => {
    ...
    expect(defaultEnabledForDriver(ProviderDriverKind.make("claudeAgent"))).toBe(true);
  ...
  it("resolves instance enabled state with explicit false winning", () => {
    ...
    expect(resolveProviderInstanceEnabled({ driver: claude, config: {} })).toBe(true);
    expect(resolveProviderInstanceEnabled({ driver: codex, config: {} })).toBe(true);
    // Envelope flag wins over the driver default.
    expect(resolveProviderInstanceEnabled({ driver: claude, enabled: true, config: {} })).toBe(
      true,
    );
    ...
    expect(resolveProviderInstanceEnabled({ driver: claude, config: { enabled: true } })).toBe(
      true,
    );
    // Conflicting flags: the explicit false wins, whichever side it is on.
    expect(
      resolveProviderInstanceEnabled({ driver: claude, enabled: true, config: { enabled: false } }),
    ).toBe(false);
    expect(
