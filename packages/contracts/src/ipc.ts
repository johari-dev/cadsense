...
import type { OpenInFileManagerInput } from "./fileManager.ts";
...
export interface DesktopPreviewWebviewConfig {
  /** `persist:cadsense-preview` (or whatever the desktop chose). */
  partition: string;
  /**
...
export interface DesktopBridge {
  ...
  /** Native local file-manager launch. Optional for older desktop shells. */
  openInFileManager?: (input: OpenInFileManagerInput) => Promise<boolean>;
  ...
  /**
   * Desktop-only preview surface. Present iff the renderer runs inside the
 ...
 * They represent backend stateful capabilities such as orchestration, project,
 * and provider operations. Each environment gets
