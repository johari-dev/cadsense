...
import { FileManagerRevealKind } from "./fileManager.ts";
 ...
 *    doesn't ship. This can happen after switching from a build that
 *    introduced a new driver. The snapshot is preserved so the UI
 *    can render "missing driver" affordances and so the data round-trips
 *    if the user returns to the other build.
 *
...
export const ServerConfig = Schema.Struct({
  ...
  /** Whether the native file manager can reveal a path. */
