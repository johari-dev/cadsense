...
//
// These tests pin the cross-version compatibility invariant: legacy payloads
// decode cleanly for externally provided drivers, and the decoded form uses
...
it.effect("ModelSelection decodes unknown driver kinds via the legacy provider field", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeModelSelection({
      provider: "ollama",
      model: "llama3:70b",
