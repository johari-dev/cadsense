...
Load this reference only when inspecting or seeding local cadsense state directly.
...
```bash
node apps/server/scripts/cadsense-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/cadsense-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/cadsense-seed.sql
```
...
Use the app's commands or APIs for behavior tests.
