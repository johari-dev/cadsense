...
>()("@cadsense/server/mcp/PreviewAutomationBroker") {}
...
type RendererDetailKind = "null" | "array" | "object" | "string" | "number" | "boolean";

function rendererDetailKind(detail: unknown): RendererDetailKind {
...
const classifyResponseError = (
  ...
  const rendererDiagnostics = {
    rendererTag: error._tag,
    rendererMessageLength: error.message.length,
    ...(error.detail === undefined ? {} : { rendererDetailKind: rendererDetailKind(error.detail) }),
    cause: error,
  };
  switch (error._tag) {
    ...
    case "PreviewAutomationTargetNotEditableError": {
      ...
      return new PreviewAutomationTargetNotEditableError({
        ...context,
        ...rendererDiagnostics,
        ...(rendererSelectorKind === undefined && context.selectorKind === undefined
          ? {}
          : { selectorKind: rendererSelectorKind ?? context.selectorKind }),
        ...(rendererSelectorLength === undefined && context.selectorLength === undefined
          ? {}
          : { selectorLength: rendererSelectorLength ?? context.selectorLength }),
      });
    }
    ...
    case "PreviewAutomationUnavailableError":
      return new PreviewAutomationRendererUnavailableError({
        ...context,
        ...rendererDiagnostics,
      });
    default:
