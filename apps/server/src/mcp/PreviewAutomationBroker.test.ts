...
it.effect("targets multiple tabs explicitly while retaining a default tab", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const simulatorTabId = PreviewTabId.make("tab-preview-browser");
...
it.effect("preserves bounded request and renderer selector diagnostics", () => {
  const locator = "role=button[name='request-secret']";
  const rendererMessage = "Unexpected token near renderer-secret.";
  const rendererError = {
    _tag: "PreviewAutomationInvalidSelectorError",
    message: rendererMessage,
    detail: { selector: "role=button[name='renderer-secret']" },
  } as const;
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        rendererTag: "PreviewAutomationInvalidSelectorError",
        rendererMessageLength: rendererMessage.length,
        rendererDetailKind: "object",
      });
      expect(error.message).toBe(
        `Preview automation click received an invalid locator (${locator.length} characters).`,
      );
      expect(error.message).not.toContain("secret");
      expect(error.cause).toBe(rendererError);
      expect("selector" in error).toBe(false);
      expect("rendererMessage" in error).toBe(false);
      expect("rendererDetail" in error).toBe(false);
    }),
  );
});

it.effect("classifies a renderer non-editable target without collapsing it to execution", () => {
  const rendererError = {
    _tag: "PreviewAutomationTargetNotEditableError",
    message: "renderer target details",
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        rendererTag: "PreviewAutomationTargetNotEditableError",
      });
...
it.effect("distinguishes malformed renderer failures", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const broker = yield* makeBroker;
      const requests = requestsFrom(yield* broker.connect(makeHost()));
