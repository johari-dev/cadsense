  ...
  readonly body: {
    readonly batch?: ReadonlyArray<{
      ...
      readonly properties?: {
        ...
        readonly cadsenseCodeVersion?: string;
      };
    }>;
  } | null;
}
  ...
  it.effect("flush drains all buffered events across multiple batches", () =>
    Effect.gen(function* () {
      ...
      const serverConfigLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
        prefix: "cadsense-telemetry-base-",
      });
      const telemetryLayer = AnalyticsService.layer.pipe(Layer.provide(serverConfigLayer));
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          CADSENSE_TELEMETRY_ENABLED: true,
          CADSENSE_POSTHOG_KEY: "phc_test_key",
          CADSENSE_POSTHOG_HOST: "http://localhost",
          CADSENSE_TELEMETRY_FLUSH_BATCH_SIZE: 20,
        }),
      );
      ...
      }).pipe(Effect.provide(Layer.merge(runtimeLayer, serverConfigLayer)));
      ...
      assert.equal(
        batchRequests.every((request) =>
          request.body.batch.every((event) => event.properties?.clientType === "desktop-app"),
        ),
        true,
      );
      assert.equal(
        batchRequests.every((request) =>
          request.body.batch.every(
            (event) =>
              ...
              event.properties.serverAppVersion === event.properties.cadsenseCodeVersion &&
              event.properties.serverMode === "desktop",
          ),
        ),
        true,
      );
