import type { ThreadId } from "@cadsense/contracts";
 ...
 * rewritten by projection rebuilds, which would silently invalidate every
 * persisted cursor with no event emitted. The (anchor, turnId) pair is derived
 * from event content, so cursors survive rebuilds and no client-side refresh
 * machinery is needed. The anchor doubles as the time bound for rows with no
 * turn linkage (straggler
