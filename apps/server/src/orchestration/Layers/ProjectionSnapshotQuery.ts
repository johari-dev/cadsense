...
// COALESCE(turn_id, ''). Both are event-derived, so cursors survive full
// projection rebuilds.
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  // turns around them. Proposed plans stay unwindowed: they
  // are metadata-scale.
    ...
    sql
      .withTransaction(
        Effect.gen(function* () {
          ...
          // rather than failing: a stale cursor after a reconnect should
          // degrade to "reload recent history", not error.
          const decodedCursor =
