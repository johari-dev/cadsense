...
// Incremental output should repaint at display cadence, not persist one database event per token.
const STREAMING_ASSISTANT_FRAME_WINDOW = Duration.millis(16);
...
interface PendingStreamingAssistantDelta {
  event: ProviderRuntimeEvent;
  threadId: ThreadId;
  messageId: MessageId;
...
const make = Effect.gen(function* () {
  ...
  const streamingAssistantDeltaMutex = yield* Semaphore.make(1);
  const streamingAssistantFlushScheduled = yield* Ref.make(false);
  const pendingStreamingAssistantDeltas = new Map<MessageId, PendingStreamingAssistantDelta>();
  ...
  const flushStreamingAssistantDeltas = streamingAssistantDeltaMutex.withPermits(1)(
    Effect.gen(function* () {
      const pending = [...pendingStreamingAssistantDeltas.values()];
      pendingStreamingAssistantDeltas.clear();
      yield* Effect.forEach(
        pending,
        (input) =>
          providerCommandId(input.event, "assistant-delta-frame").pipe(
  ...
  const hasPendingStreamingAssistantDeltas = streamingAssistantDeltaMutex.withPermits(1)(
    Effect.sync(() => pendingStreamingAssistantDeltas.size > 0),
  );

  function scheduleStreamingAssistantFlush(): Effect.Effect<void, never, Scope.Scope> {
    return Effect.gen(function* () {
      const shouldSchedule = yield* Ref.modify(streamingAssistantFlushScheduled, (scheduled) =>
        scheduled ? [false, true] : [true, true],
      );
      if (!shouldSchedule) return;

      yield* forkParked(
        Effect.sleep(STREAMING_ASSISTANT_FRAME_WINDOW).pipe(
          Effect.andThen(flushStreamingAssistantDeltas),
          Effect.catchCause((cause) =>
            Effect.logWarning("failed to flush streaming assistant frame", {
              cause: Cause.pretty(cause),
            }),
          ),
          Effect.ensuring(
            Ref.set(streamingAssistantFlushScheduled, false).pipe(
              Effect.andThen(
                hasPendingStreamingAssistantDeltas.pipe(
                  Effect.flatMap((hasPending) =>
                    hasPending ? scheduleStreamingAssistantFlush() : Effect.void,
                  ),
                ),
              ),
            ),
  ...
  const queueStreamingAssistantDelta = (input: PendingStreamingAssistantDelta) =>
    streamingAssistantDeltaMutex
      .withPermits(1)(
        Effect.sync(() => {
          const existing = pendingStreamingAssistantDeltas.get(input.messageId);
          pendingStreamingAssistantDeltas.set(
            input.messageId,
            existing === undefined
              ? input
              : {
                  ...existing,
                  delta: `${existing.delta}${input.delta}`,
                  createdAt: input.createdAt,
                },
          );
        }),
      )
      .pipe(Effect.andThen(scheduleStreamingAssistantFlush()));
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      const isStreamingAssistantDelta =
        event.type === "content.delta" &&
        event.payload.streamKind === "assistant_text" &&
        (yield* serverSettingsService.getSettings).streamAssistantText;
      if (!isStreamingAssistantDelta) {
        yield* flushStreamingAssistantDeltas;
      }
      ...
      // steering a running turn can make providers open a
      ...
      if (assistantDelta && assistantDelta.length > 0) {
        ...
        const assistantDeliveryMode: AssistantDeliveryMode = yield* Effect.map(
          ...
          (settings) => (settings.streamAssistantText ? "streaming" : "buffered"),
        );
  ...
  return {
    ...
    drain: worker.drain.pipe(
      Effect.andThen(flushStreamingAssistantDeltas),
      Effect.catchCause((cause) =>
        Effect.logWarning("failed to flush streaming assistant deltas while draining", {
          cause: Cause.pretty(cause),
        }),
      ),
    ),
