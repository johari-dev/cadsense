...
async function createOrchestrationSystem() {
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "cadsense-orchestration-engine-test-",
  });
...
describe("OrchestrationEngine", () => {
    ...
    const layer = OrchestrationEngineLive.pipe(
      Layer.provide(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          listPendingOnshapeProjects: () => Effect.succeed([]),
  ...
  it("records command ack duration using the first committed event type", async () => {
    ...
    expect(
      hasMetricSnapshot(snapshots, "cadsense_orchestration_command_ack_duration", {
  ...
  it("records failed command dispatches as metric failures", async () => {
    ...
    const snapshots = await system.run(Metric.snapshot);
    expect(
      hasMetricSnapshot(snapshots, "cadsense_orchestration_commands_total", {
        commandType: "thread.create",
        aggregateKind: "thread",
        outcome: "failure",
      }),
  ...
  it("stamps the dispatching client's origin onto persisted event metadata", async () => {
    ...
    await system.run(
      engine.dispatch(
        ...
        { origin: { surface: "desktop", appVersion: "1.2.3" } },
      ),
    );
    await system.run(
      engine.dispatch({
    ...
    expect(withOrigin?.metadata.origin).toEqual({ surface: "desktop", appVersion: "1.2.3" });
