...
function makeTestServerSettingsLayer(overrides: Partial<ServerSettings> = {}) {
  return ServerSettingsService.layerTest({ streamAssistantText: false, ...overrides });
}
...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("accepts a conflicting turn.started for a pending turn start when the provider expects that turn", async () => {
    ...
    // one is still active, and a provider can open the new turn
  ...
  it("starts a new streaming assistant message segment after approval", async () => {
    const harness = await createHarness({ serverSettings: { streamAssistantText: true } });
  ...
  it("projects Codex task lifecycle chunks into thread activities", async () => {
    ...
    harness.emit({
      ...
      payload: {
        ...
        summary: "Validation agent is checking the desktop rollout chunks.",
      },
    });
    ...
    expect(progressPayload?.detail).toBe(
      "Validation agent is checking the desktop rollout chunks.",
    );
  ...
  it("titles task activities with the task description, including on completion", async () => {
    ...
    harness.emit({
      ...
      payload: {
        taskId: "named-task-1",
        description: "Typecheck desktop app",
        summary: "Running tsc across the desktop workspace.",
      },
    });
    ...
    expect(progress?.summary).toBe("Typecheck desktop app");
    expect(progressPayload?.title).toBe("Typecheck desktop app");
    expect(completed?.summary).toBe("Task completed");
    expect(completedPayload?.title).toBe("Typecheck desktop app");
  ...
  it("titles task completion from task.started when no progress event carried the name", async () => {
    ...
    harness.emit({
      ...
      payload: {
        taskId: "fast-task-1",
        description: "wait for codex analysis to finish",
        taskType: "local_bash",
      },
    });
    ...
    expect(completedPayload?.title).toBe("wait for codex analysis to finish");
  });
