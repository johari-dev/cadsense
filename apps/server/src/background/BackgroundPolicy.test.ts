...
describe("BackgroundPolicy", () => {
  it.effect("records foreground scoped client demand", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(snapshot.activeScopeKeys, ["diagnostics"]);
      assert.equal(snapshot.shouldRunOpportunisticWork, true);
      assert.equal(yield* policy.hasDemand({ type: "diagnostics" }), true);
      assert.equal(yield* policy.hasDemand({ type: "server-config" }), false);
      assert.equal(yield* policy.shouldRunScopeWork({ type: "diagnostics" }), true);
      assert.equal(yield* policy.shouldRunScopeWork({ type: "server-config" }), false);
  ...
  it.effect("host low power mode disables opportunistic work without dropping scoped demand", () =>
    Effect.gen(function* () {
      ...
      assert.equal(snapshot.shouldRunOpportunisticWork, false);
      assert.equal(yield* policy.hasDemand({ type: "diagnostics" }), true);
      assert.equal(yield* policy.shouldRunScopeWork({ type: "diagnostics" }), false);
