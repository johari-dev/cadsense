...
} from "@cadsense/shared/backgroundActivitySettings";
...
>()("@cadsense/server/background/BackgroundPolicy") {}
...
function computeSnapshot(input: {
  ...
  return {
    ...
    shouldRunOpportunisticWork:
      foregroundLeases.length > 0 && !isHostConstrained(input.hostPower, input.settings),
