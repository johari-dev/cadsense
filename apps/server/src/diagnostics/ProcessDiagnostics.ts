...
>()("@cadsense/server/diagnostics/ProcessDiagnostics") {}
...
function canSignalCategory(category: ResourceTelemetryProcessCategory): boolean {
  return category === "server-child" || category === "provider-root";
}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the Cadsense server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signaled: false,
          message: Option.some(
            `Process ${input.pid} is not a signalable cadsense backend descendant.`,
          ),
        };
      }
