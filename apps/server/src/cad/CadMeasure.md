...
A positive surface distance does not exclude solid containment. Zero does not distinguish touching surfaces from intersecting surfaces. Measuring an occurrence against itself returns zero when its geometry is measurable. Tessellation error is unknown, so the value is not a manufacturing tolerance, signed clearance, or penetration depth.
...
- 128 MiB per cached GLB and per declared decoded geometry, 10,000 visited GLB nodes, and 100,000 triangles per occurrence.
...
Known asset complexity is checked before reading geometry. Tree construction and search yield between work chunks so other requests and cancellation can run.
