  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const loadServerConfig = Effect.gen(function* () {
        ...
        const fileManagerRevealKind = yield* resolveFileManagerRevealKindForConfig(
      ...
      return WsRpcGroup.of({
            ...
            Effect.gen(function* () {
              ...
              const shouldStopSessionAfterCommand = archiveCommand
                ? yield* projectionSnapshotQuery.getThreadShellById(archiveCommand.threadId).pipe(
                    ...
                    Effect.catchCause((cause) =>
                      Effect.logWarning(
                        "failed to read thread session state before archive cleanup",
        ...
        [WS_METHODS.shellOpenInFileManager]: (input) =>
          observeRpcEffect(
            WS_METHODS.shellOpenInFileManager,
            externalLauncher.openInFileManager(input),
            { "rpc.aggregate": "workspace" },
          ),
