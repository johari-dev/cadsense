...
export const makeCodexTextGeneration = Effect.fn("makeCodexTextGeneration")(function* (
  ...
  const writeTempFile = (
    ...
    prefix: string,
    content: string,
  ): Effect.Effect<string, ThreadTitleGenerationError, Scope.Scope> =>
    fileSystem
      .makeTempFileScoped({
        prefix: `cadsense-${prefix}-${process.pid}-`,
      })
      .pipe(
  ...
  const materializeImageAttachments = Effect.fn("materializeImageAttachments")(function* (
    attachments: TextGeneration.ThreadTitleGenerationInput["attachments"],
  ): Effect.fn.Return<MaterializedImageAttachments, ThreadTitleGenerationError> {
