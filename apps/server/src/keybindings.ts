...
import { fromJsonStringPretty, fromLenientJson } from "@cadsense/shared/schemaJson";
import {
  DEFAULT_KEYBINDINGS,
...
} from "@cadsense/shared/keybindings";
...
const LegacyKeybindingEntry = Schema.Struct({ command: Schema.String });
...
function isRemovedKeybindingEntry(entry: unknown): boolean {
  const decoded = decodeLegacyKeybindingEntryExit(entry);
  if (decoded._tag === "Failure") return false;
  const { command } = decoded.value;
  return (
    command.startsWith("terminal.") ||
    (command.startsWith("script.") && command.endsWith(".run")) ||
    REMOVED_KEYBINDING_COMMANDS.has(command)
  );
}
...
const make = Effect.gen(function* () {
  ...
  > {
    ...
    return yield* Effect.forEach(rawConfig, (entry) =>
      Effect.gen(function* () {
        if (isRemovedKeybindingEntry(entry)) return null;
  ...
  > {
    if (!(yield* readConfigExists)) {
      return { keybindings: [], issues: [], removedEntryCount: 0 };
    }
    ...
    for (const [index, entry] of decodedEntries.value.entries()) {
      if (isRemovedKeybindingEntry(entry)) {
        removedEntryCount += 1;
        continue;
      }
      const decodedRule = decodeKeybindingRuleExit(entry);
    ...
    return { keybindings, issues, removedEntryCount };
  });
  ...
  const syncDefaultKeybindingsOnStartup = upsertSemaphore.withPermits(1)(
    Effect.gen(function* () {
      ...
      if (missingDefaults.length === 0 && runtimeConfig.removedEntryCount === 0) {
        yield* Cache.invalidate(resolvedConfigCache, resolvedConfigCacheKey);
        return;
      }
      ...
      if (defaultsToAppend.length === 0 && runtimeConfig.removedEntryCount === 0) {
        yield* Cache.invalidate(resolvedConfigCache, resolvedConfigCacheKey);
        return;
      }
