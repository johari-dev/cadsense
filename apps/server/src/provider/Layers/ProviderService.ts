...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  /**
   * Attach the `cadsense` MCP server to the session that is about to start.
   *
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in cadsense settings.`,
          );
        }
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    // and each adapter decides whether to also send native image or file parts.
    // Unresolvable ids are skipped here
  ...
  return {
    ...
    // consumers such as ProviderRuntimeIngestion each
