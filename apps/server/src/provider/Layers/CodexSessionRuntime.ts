...
const SLASH_SKILL_REFERENCE_REGEX = /(^|\s)\/([a-zA-Z][a-zA-Z0-9:_-]*)(?=\s|$)/g;

export function extractCodexSkillInputs(
  prompt: string,
  availableSkills: ReadonlyArray<CodexSkillReference>,
): {
  readonly prompt: string;
  readonly skills: ReadonlyArray<EffectCodexSchema.V2TurnStartParams__UserInput>;
} {
  const skillsByName = new Map(
    availableSkills.filter((skill) => skill.enabled).map((skill) => [skill.name, skill] as const),
  );
  const selectedSkills = new Map<string, CodexSkillReference>();
  const remainingPrompt = prompt.replace(
    SLASH_SKILL_REFERENCE_REGEX,
    (source, prefix: string, name: string) => {
      const skill = skillsByName.get(name);
      if (!skill) {
        return source;
      }
      selectedSkills.set(skill.path, skill);
      return prefix;
    },
  );
  ...
  return {
    ...
    skills: Array.from(selectedSkills.values(), (skill) => ({
      type: "skill" as const,
      name: skill.name,
      path: skill.path,
    })),
...
/** Converts a cadsense approval decision into the MCP elicitation wire response. */
