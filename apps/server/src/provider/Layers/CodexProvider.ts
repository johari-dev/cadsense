...
import { PREFERRED_DEFAULT_CODEX_MODELS, ServerSettingsError } from "@cadsense/contracts";

import { createModelCapabilities } from "@cadsense/shared/model";
...
export function buildCodexInitializeParams(): CodexSchema.V1InitializeParams {
  return {
    clientInfo: {
      name: "cadsense_desktop",
      title: "cadsense Desktop",
    ...
    if (!codexSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Codex is disabled in cadsense settings.",
        },
      });
    }
