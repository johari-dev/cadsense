...
import { DEFAULT_MODEL, ThreadId } from "@cadsense/contracts";
...
describe("buildTurnStartParams", () => {
  it("turns slash skill references anywhere in a prompt into native skill inputs", () => {
    const resolved = extractCodexSkillInputs(
      "Please /frontend-design polish this with /github:fix-ci",
      [
        {
        ...
        {
          name: "github:fix-ci",
          path: "/skills/github-fix-ci/SKILL.md",
          enabled: true,
        },
      ],
    );

    NodeAssert.equal(resolved.prompt, "Please  polish this with ");
    NodeAssert.deepStrictEqual(resolved.skills, [
      {
        type: "skill",
        name: "frontend-design",
        path: "/skills/frontend-design/SKILL.md",
  ...
  it("leaves unknown, disabled, and dollar-prefixed references as plain text", () => {
    const prompt = "Use /unknown /disabled and $frontend-design";
    const resolved = extractCodexSkillInputs(prompt, [
      {
        name: "disabled",
    ...
    NodeAssert.equal(resolved.prompt, prompt);
    NodeAssert.deepStrictEqual(resolved.skills, []);
  });

  it("includes native skill inputs in the Codex turn", () => {
...
describe("buildCodexDeveloperInstructions", () => {
  it("appends runtime info after the mode instructions", () => {
    ...
    NodeAssert.match(instructions, /cadsense/);
...
describe("cadsense browser developer instructions", () => {
  ...
  it("omits the browser block entirely when the preview tools are not attached", () => {
    for (const instructions of [
      ...
      NodeAssert.doesNotMatch(instructions, /cadsense collaborative browser/);
...
describe("hasConfiguredMcpServer", () => {
  it("detects inline Codex MCP configuration arguments", () => {
    ...
    NodeAssert.equal(
      hasConfiguredMcpServer(["-c", 'mcp_servers.cadsense.url="http://127.0.0.1/mcp"']),
      true,
    );
  });
});
  ...
  it("suppresses memory consolidation without hiding other Codex subagents", () => {
    ...
    NodeAssert.equal(
      shouldSuppress({
        ...
        params: {
          threadId: "memory-thread",
          threadName: "internal memory update",
        },
      }),
      true,
    );
  ...
  it("keeps launch args when explicit app-server args are provided", () => {
    NodeAssert.deepStrictEqual(
      codexSessionAppServerArgs(
        ["-c", "mcp_servers.cadsense.url=http://127.0.0.1/mcp"],
      ...
      [
        ...
        "foo",
        "-c",
        "mcp_servers.cadsense.url=http://127.0.0.1/mcp",
      ],
    );
  });
});
