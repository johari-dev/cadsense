...
it.layer(NodeServices.layer)("discoverClaudeSkills", (it) => {
  it.effect("discovers user and project skills with frontmatter metadata", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "cadsense-claude-skills-" });
  ...
  it.effect("lets the administrator's managed policy outrank every other settings file", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        path.join("C:/ProgramData", "ClaudeCode", "managed-settings.json"),
      );
      ...
      assert.deepEqual(
        ...
        [
          path.join("/home/.claude", "settings.json"),
          path.join("/repo/packages/app", ".claude", "settings.json"),
          path.join("/repo/packages/app", ".claude", "settings.local.json"),
          path.join("/repo", ".claude", "settings.local.json"),
