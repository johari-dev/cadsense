...
const descriptions = {
  ...
  cad_model_diagnostics:
    "Read bounded diagnostics from the selected pinned snapshot at {expectedRevision,snapshotId}. Reports unsupported and suppressed components, expected unloaded geometry, missing manifest associations or metadata, and stored mesh costs. Optional minimumSeverity is info, warning, or error; use warning to omit expected suppression and unloading messages. Set thresholds with triangles, decodedBytes, and drawCalls. Defaults are 1000000 triangles,134217728 decoded bytes,128 draw calls. limit is 1..100; follow nextCursor with unchanged snapshot, revision, severity, and thresholds. No asset files or remote sources are checked by this scan. Inspect coverage and unknown counts before interpreting known subtotals. Potential assembled workload includes all unsuppressed instances independent of visibility. Cost findings link only unsuppressed part occurrences. Cost warnings do not establish engineering defects.",
  cad_hierarchy:
