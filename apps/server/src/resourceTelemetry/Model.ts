...
export interface TelemetryCounters {
  ...
  readonly allCadsense: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allCadsense: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allCadsense: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allCadsense = input.counters.allCadsense;
  for (const processDelta of input.deltas) {
    ...
    allCadsense = incrementCounters(allCadsense, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allCadsense = incrementCounters(allCadsense, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allCadsense = incrementCounters(allCadsense, { processExits: 1 });
  }

  return { backend, electron, monitor, allCadsense };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allCadsense: aggregate(ordered, counters.allCadsense),
    },
    deltas: processDeltas,
  };
}
