...
import { type DeepPartial, deepMerge } from "@cadsense/shared/Struct";
import { fromJsonStringPretty, fromLenientJson } from "@cadsense/shared/schemaJson";
import {
  applyServerSettingsPatch,
  isModelSelectionProviderEnabled,
} from "@cadsense/shared/serverSettings";
...
>()("@cadsense/server/serverSettings/ServerSettingsService") {
...
const makeTest = (overrides: DeepPartial<ServerSettings> = {}) =>
  Effect.gen(function* () {
    const { providerHealthRefreshInterval, ...overridesForMerge } = overrides;
...
const make = Effect.gen(function* () {
  ...
  const loadSettingsFromDisk = Effect.gen(function* () {
    ...
    return foldProviderInstanceEnabledFlags(settings);
  });
