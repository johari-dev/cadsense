  ...
  yield* withDatabase(
    ...
    Effect.gen(function* () {
      ...
      yield* sql`INSERT INTO projection_projects
        (project_id, title, workspace_root, created_at, updated_at, deleted_at)
        VALUES
      ...
      const threads = [
        ["stopped-thread", "project-kept", "stopped"],
        ["running-thread", "project-kept", "running"],
        ["deleted-project-thread", "project-deleted", "stopped"],
      ] as const;
      for (const [threadId, projectId, status] of threads) {
        yield* sql`INSERT INTO projection_threads
          (thread_id, project_id, title, created_at, updated_at)
          VALUES (${threadId}, ${projectId}, ${threadId}, '2026-08-01', '2026-08-01')`;
...
it.layer(NodeServices.layer)("migrate-dev-db", (it) => {
  it.effect("keeps only stopped threads from live projects", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
