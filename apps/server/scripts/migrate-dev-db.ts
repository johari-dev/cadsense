 ...
 * ~/.cadsense database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a local checkout:
 *   1. Replaces `<workspace>/.cadsense/userdata/state.sqlite`.
 ...
 *      threads that have fully stopped. Active threads are skipped so the
 *      dev server never adopts live work.
 *      Command receipts and provider runtime rows are dropped.
...
// @effect-diagnostics nodeBuiltinImport:off - node:os resolves the shared cadsense home guard.
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedErrorClass<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.cadsense database. Use an isolated --base-dir.";
  }
}
...
export class MigrateDevDbServerRunningError extends Schema.TaggedErrorClass<MigrateDevDbServerRunningError>()(
  ...
  override get message(): string {
    return `Dev database at '${this.databasePath}' is open by a running server (pid ${this.pid} per server-runtime.json). Stop that server first; if that pid is not actually a Cadsense server (stale descriptor, reused pid), delete the server-runtime.json next to the database and retry.`;
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .cadsense directory. Defaults to `<workspace>/.cadsense`. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.cadsense/userdata/state.sqlite`. */
...
const pruneSnapshot = Effect.fn("pruneDevDbSnapshot")(function* (input: RunMigrateDevDbInput) {
  ...
  // Keep only threads whose provider session has fully stopped. In-memory
  // activity never persists, so filtering the session status is sufficient.
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".cadsense"));
  const sourcePath = path.resolve(
    input.source ?? path.join(sharedHome, "userdata", "state.sqlite"),
  );

  const baseDir = path.resolve(input.baseDir ?? path.join(process.cwd(), ".cadsense"));
  ...
  const { executedMigrations, pruned } = yield* Effect.gen(function* () {
    ...
    // collision must abort before the old workspace db gets replaced with a
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .cadsense directory. Defaults to the current workspace's .cadsense.",
      ),
    ),
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.cadsense/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the workspace dev database from a pruned snapshot of the real ~/.cadsense data, then run migrations.",
  ),
);
