...
import { fromJsonStringPretty } from "@cadsense/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Cadsense once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.cadsense database. Use an isolated --base-dir.";
  }
}
...
export const cadsenseSqliteStateCommand = Command.make(
  "cadsense-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Cadsense base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Cadsense SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(cadsenseSqliteStateCommand, { version: "0.0.0" }).pipe(
