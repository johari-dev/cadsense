...
it.live("runs a single turn end-to-end and persists the projected conversation", () =>
  withHarness((harness) =>
    Effect.gen(function* () {
...
it.live("runs multi-turn file edits while preserving conversation state", () =>
  withHarness((harness) =>
    Effect.gen(function* () {
      yield* seedProjectAndThread(harness);
      ...
      const secondTurnThread = yield* harness.waitForThread(
        ...
        (entry) =>
          ...
          entry.latestTurn.state === "completed" &&
          entry.messages.some(
            (message) => message.role === "assistant" && message.text.includes("v3"),
          ),
      );
      assert.equal(
        secondTurnThread.messages.some(
          (message) => message.role === "assistant" && message.text.includes("v2"),
        ),
        true,
      );
      assert.equal(
...
it.live("records failed turn runtime state as error", () =>
  withHarness((harness) =>
    Effect.gen(function* () {
      ...
      assert.equal(
        thread.activities.some((activity) => activity.kind === "runtime.error"),
        true,
      );
    }),
  ),
