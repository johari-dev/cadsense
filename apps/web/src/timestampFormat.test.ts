...
describe("getTimestampFormatOptions", () => {
  it("always builds a 12-hour formatter", () => {
    expect(getTimestampFormatOptions(true)).toEqual({
      hour: "numeric",
      minute: "2-digit",
      second: "2-digit",
      hour12: true,
    });
    expect(getTimestampFormatOptions(false)).toEqual({
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
    });
...
describe("resolveTimestampLocale", () => {
  ...
  it("keeps the 12-hour cycle for every host locale", () => {
    ...
    expect(formatAt1544("en-GB")).toMatch(/^3:44\s+pm$/i);
...
describe("formatDayAwareTimestamp", () => {
  ...
  const time = (isoDate: string) => formatShortTimestamp(isoDate);

  it("shows time only for today", () => {
    const messageAt = iso(2026, 7, 14, 9, 30);
    expect(formatDayAwareTimestamp(messageAt, now)).toBe(time(messageAt));
  });
  ...
  it("labels the previous calendar day as yesterday even when under 24h old", () => {
    ...
    expect(formatDayAwareTimestamp(messageAt, justPastMidnight)).toBe(
  ...
  it("prefixes older same-year messages with the numeric date", () => {
    ...
    expect(formatDayAwareTimestamp(messageAt, now)).toBe(`${datePart} ${time(messageAt)}`);
  });
  ...
  it("uses the host locale for both the numeric date and wall-clock time", async () => {
    ...
    expect(formatWithHostLocale(messageAt, now)).toMatch(/^12\/08 3:44\s+pm$/i);

    vi.unstubAllGlobals();
  });

  it("returns an empty string for invalid input", () => {
    expect(formatDayAwareTimestamp("not-a-date", now)).toBe("");
  });
});
