...
import { DEFAULT_CLIENT_SETTINGS } from "@cadsense/contracts/settings";
...
describe("mergeEnvironmentSettings", () => {
  it("combines the selected environment's server settings with client preferences", () => {
    const serverSettings = {
      ...DEFAULT_SERVER_SETTINGS,
      providerInstances: {
        [ProviderInstanceId.make("codex_secondary")]: {
    ...
    const clientSettings = {
      ...
      favorites: [
        {
          provider: ProviderInstanceId.make("codex_secondary"),
          model: "gpt-5.4",
        },
      ],
    };
