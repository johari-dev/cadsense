export function getTimestampFormatOptions(includeSeconds: boolean): Intl.DateTimeFormatOptions {
  return {
    hour: "numeric",
    minute: "2-digit",
...
function getTimestampFormatter(includeSeconds: boolean): Intl.DateTimeFormat {
  const cacheKey = includeSeconds ? "seconds" : "minutes";
  ...
  const formatter = new Intl.DateTimeFormat(
    timestampLocale,
    getTimestampFormatOptions(includeSeconds),
  );
 ...
 */
export function formatChatTimestampTooltip(isoDate: string): string {
  const date = parseTimestampDate(isoDate);
  if (!date) return "";
  const time = formatShortTimestamp(isoDate);
...
export function formatShortTimestamp(isoDate: string): string {
  const date = parseTimestampDate(isoDate);
  if (!date) return "";
  return getTimestampFormatter(false).format(date);
}
 ...
 */
export function formatDayAwareTimestamp(isoDate: string, nowMs: number = Date.now()): string {
  const date = parseTimestampDate(isoDate);
  if (!date) return "";
  const time = getTimestampFormatter(false).format(date);
