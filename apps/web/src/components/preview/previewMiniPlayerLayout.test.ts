import { FILL_PREVIEW_VIEWPORT } from "@cadsense/contracts";
...
describe("clampPreviewMiniPlayerSize", () => {
  it("allows resizing within the available chat viewport", () => {
    expect(clampPreviewMiniPlayerSize({ width: 520, height: 360 }, container, 120)).toEqual({
      width: 520,
      height: 360,
    });
  });
  ...
  it("bounds oversized players above the composer", () => {
    expect(clampPreviewMiniPlayerSize({ width: 2_000, height: 2_000 }, container, 120)).toEqual({
      width: 976,
      height: 556,
    });
  });
