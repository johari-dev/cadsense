...
import { formatProviderSkillDisplayName } from "@cadsense/client-runtime/providerSkills";
...
function ComposerMentionDecorator(props: { path: string }) {
  const chip = (
    ...
    >
      <FileTagChipContent path={props.path} label={basenameOfPath(props.path)} />
    </span>
  );
...
type ComposerInlineTokenNode = ComposerMentionNode | ComposerSkillNode;

function isComposerInlineTokenNode(candidate: unknown): candidate is ComposerInlineTokenNode {
  return candidate instanceof ComposerMentionNode || candidate instanceof ComposerSkillNode;
}
...
function $setComposerEditorPrompt(
  ...
  for (const segment of segments) {
    ...
    if (segment.type === "skill") {
      ...
      if (!metadata) {
        $appendTextWithLineBreaks(paragraph, `/${segment.name}`);
        continue;
      }
      paragraph.append(
...
export function ComposerPromptEditor({
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "cadsense-composer-editor",
      editable: true,
