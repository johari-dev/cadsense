...
describe("canUseMarkdownFileShellActions", () => {
  ...
  it("allows Explorer actions when an environment owns the file", () => {
    expect(canUseMarkdownFileShellActions(environmentId)).toBe(true);
  });

  it("hides shell actions when no environment owns the markdown", () => {
    expect(canUseMarkdownFileShellActions(null)).toBe(false);
  });
});

describe("hasMarkdownFilePrimaryAction", () => {
  it("keeps the chip interactive when Explorer, browser, or panel can open it", () => {
    expect(
      hasMarkdownFilePrimaryAction({
...
describe("shouldUseMarkdownFileBrowserPrimaryAction", () => {
  ...
  it("preserves panel and Explorer defaults for HTML files", () => {
    expect(
      shouldUseMarkdownFileBrowserPrimaryAction({
        iconPath: "/tmp/report.html",
        canOpenInBrowser: true,
