...
function categoryLabel(category: ResourceTelemetryProcessCategory): string {
  switch (category) {
    ...
    case "resource-monitor":
      return "Monitor";
    case "unknown-cadsense":
      return "cadsense process";
  }
}
...
function canSignalProcess(process: ResourceTelemetryProcess): boolean {
  return process.category === "server-child" || process.category === "provider-root";
}
...
export function ResourceTelemetryDiagnostics() {
  ...
  const allCadsense = snapshot?.groups.allCadsense;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          <div className="flex flex-col gap-3 border-b border-border/60 bg-linear-to-r from-muted/45 via-muted/20 to-transparent px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">
                cadsense system footprint
              </div>
              <p className="mt-1 max-w-xl text-xs leading-relaxed text-muted-foreground">
                Live native counters for the server, providers, desktop processes, and the monitor
                itself.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allCadsense ? `${allCadsense.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={
                allCadsense
                  ? `${formatCpuTime(allCadsense.cpuTimeMs)} observed CPU time`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allCadsense ? formatBytes(allCadsense.currentRssBytes) : "..."}
              detail={
                allCadsense
                  ? `${formatBytes(allCadsense.peakRssBytes)} combined process peaks`
                  : undefined
              }
            />
            <IconStat
              ...
              label="Process count"
              value={allCadsense ? String(allCadsense.processCount) : "..."}
              detail={
                allCadsense
                  ? `${allCadsense.processStarts} starts · ${allCadsense.processExits} exits`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allCadsense ? formatRate(allCadsense.ioReadBytesPerSecond) : "..."}
              detail={allCadsense ? `${formatBytes(allCadsense.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allCadsense ? formatRate(allCadsense.ioWriteBytesPerSecond) : "..."}
              detail={allCadsense ? `${formatBytes(allCadsense.ioWriteBytes)} observed` : undefined}
              tone={
                allCadsense && allCadsense.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allCadsense && allCadsense.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known cadsense operations so process spikes can be correlated with
