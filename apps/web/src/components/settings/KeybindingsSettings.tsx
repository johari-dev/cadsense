...
function UnknownWhenVariableWarning({
  ...
  return (
    <WarningTooltipIcon label={label} focusable={focusable} className="size-4.5">
      cadsense does not recognize this condition yet. It can still be saved, but it may not match
...
function BrowserKeybindingNotice() {
  return (
    <div className="flex items-center gap-1.5 px-3 pb-2 text-[12px] text-muted-foreground sm:px-4">
      ...
      <span>
        Some shortcuts may be claimed by the browser before cadsense sees them. Use the desktop app
  ...
  const openKeybindingsFile = useCallback(() => {
    ...
    void (async () => {
      const result = await openInFileManager({
        ...
        input: { cwd: keybindingsConfigPath, reveal: true },
      });
      ...
      toastManager.add({
        ...
        description: error instanceof Error ? error.message : "The keybindings file was not shown.",
        type: "error",
      });
    })();
  }, [keybindingsConfigPath, openInFileManager, primaryEnvironment]);
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    aria-label="Show keybindings.json in Explorer"
                  >
                    <FileJsonIcon className="size-3" />
                  </Button>
                }
              />
              <TooltipPopup side="top">Show keybindings.json in Explorer</TooltipPopup>
            </Tooltip>
          </div>
        }
      >
