...
import { parseScopedThreadKey } from "@cadsense/client-runtime/environment";
import { commandProgramName } from "@cadsense/client-runtime/work-log/command-label";
import type { AgentPanelModel } from "@cadsense/client-runtime/state/subagentRuntime";
import {
...
import { resolveChatListAnchoredEndSpace } from "@cadsense/shared/chatList";
import {
  createContext,
  Fragment,
  memo,
...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  let visibleText = row.message.text;
  while (true) {
  ...
  const elementContexts = elementContextState.contexts;
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Tooltip>
            <TooltipTrigger render={<p className="text-muted-foreground text-xs tabular-nums" />}>
              {formatDayAwareTimestamp(row.message.createdAt)}
            </TooltipTrigger>
            <TooltipPopup>{formatChatTimestampTooltip(row.message.createdAt)}</TooltipPopup>
          </Tooltip>
          ...
          <div className="mt-1.5 flex items-center gap-2 text-xs tabular-nums opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover/assistant:opacity-100">
            ...
            {!row.message.streaming && (
              <Tooltip>
                ...
                >
                  {formatDayAwareTimestamp(row.message.updatedAt)}
                </TooltipTrigger>
                <TooltipPopup>{formatChatTimestampTooltip(row.message.updatedAt)}</TooltipPopup>
              </Tooltip>
            )}
          </div>
        ) : null}
