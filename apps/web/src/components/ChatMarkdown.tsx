...
} from "@cadsense/client-runtime/markdown-images";
...
export function canUseMarkdownFileShellActions(environmentId: EnvironmentId | null): boolean {
...
export function hasMarkdownFilePrimaryAction(input: {
  ...
  return input.canOpenInBrowser || input.canOpenInPanel || input.canReveal;
}
...
export function shouldUseMarkdownFileBrowserPrimaryAction(input: {
  ...
  return (
    ...
    (shouldOpenMarkdownFileLinkInBrowserByDefault(input.iconPath) ||
      (!input.canOpenInPanel && !input.canReveal))
  );
}
...
function MarkdownTable({ children, ...props }: React.ComponentProps<"table">) {
  ...
  return (
    <div ref={containerRef} className="chat-markdown-table-container" data-expanded="true">
      <ScrollArea
        chainVerticalScroll
        scrollFade
        hideScrollbars
 ...
 * "Open in editor" button — already shows its identity, and a favicon bolted on in front of it
...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  }, [handleRevealInFileManager, line, onOpenInPanel, threadRef, workspaceRelativePath]);
  ...
  const canReveal = onReveal !== undefined;
...
function ChatMarkdown({
  ...
  const canUseShellActions = canUseMarkdownFileShellActions(environmentId);
  ...
  const revealInFileManagerLabel =
    environmentId !== null && serverConfig
    ...
    (filePath: string) => {
      if (environmentId === null) {
        return Promise.resolve(
          AsyncResult.failure<void, Error>(Cause.fail(new Error("No environment owns this file."))),
        );
      }
      return openInFileManager({
        environmentId,
        input: { cwd: filePath, reveal: true },
      });
    },
