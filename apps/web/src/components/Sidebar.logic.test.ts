...
describe("getSidebarThreadIdsToPrewarm", () => {
  it("returns only the first visible thread ids up to the prewarm limit", () => {
    expect(getSidebarThreadIdsToPrewarm(["t1", "t2", "cadsense"], 2)).toEqual(["t1", "t2"]);
  });
...
describe("orderItemsByPreferredIds", () => {
  ...
  it("honors projectOrder physical keys via getProjectOrderKey", async () => {
    ...
    const { getProjectOrderKey } = await import("../sidebarProjects");
    const projects = [
      {
        environmentId: EnvironmentId.make("environment-local"),
...
describe("searchSidebarThreadsByTitle", () => {
  const threads = [
    ...
    { id: "thread-3", title: "WORKFLOW cleanup", project: "Beta" },
  ];
...
describe("resolveThreadStatusPill", () => {
  ...
  it("shows plan ready when a completed plan turn has a proposed plan ready for follow-up", () => {
    expect(
      resolveThreadStatusPill({
        thread: {
          ...baseThread,
