...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "cadsense:last-invoked-script-by-project";
...
export function resolveThreadMetadataUpdateForNextTurn(input: {
  ...
  return modelSelectionChanged ? { modelSelection: nextModelSelection } : {};
}
...
export function deriveComposerSendState(options: {
  ...
  imageCount: number;
  /**
   * Optional element-pick attachment count. A prompt of just element chips is
   * still valid to send.
   */
...
// fallback behavior — the routing layer is the right place to surface
