...
const CUSTOM_INSTANCE = ProviderInstanceId.make("custom");
const CODEX_DRIVER = ProviderDriverKind.make("codex");
const CLAUDE_AGENT_DRIVER = ProviderDriverKind.make("claudeAgent");
const CUSTOM_DRIVER = ProviderDriverKind.make("custom");
...
const OTHER_TEST_ENVIRONMENT_ID = EnvironmentId.make("environment-secondary");
...
describe("composerDraftStore project draft thread mapping", () => {
  ...
  const secondaryProjectRef = scopeProjectRef(OTHER_TEST_ENVIRONMENT_ID, projectId);
  ...
  const secondaryDraftId = DraftId.make("draft-secondary");
  ...
  it("clears composer data for one environment without touching another", () => {
    ...
    const secondaryThreadRef = scopeThreadRef(OTHER_TEST_ENVIRONMENT_ID, otherThreadId);
    ...
    try {
      ...
      store.setProjectDraftThreadId(secondaryProjectRef, secondaryDraftId, {
        threadId: otherThreadId,
      });
      store.setPrompt(localDraftId, "local draft");
      store.setPrompt(secondaryDraftId, "secondary draft");
      ...
      store.setPrompt(secondaryThreadRef, "secondary thread draft");
      ...
      expect(next.getDraftThreadByProjectRef(secondaryProjectRef)).not.toBeNull();
      expect(next.getComposerDraft(localDraftId)).toBeNull();
      expect(next.getComposerDraft(secondaryDraftId)?.prompt).toBe("secondary thread draft");
      expect(next.getComposerDraft(localThreadRef)).toBeNull();
      expect(next.getComposerDraft(secondaryThreadRef)?.prompt).toBe("secondary thread draft");
  ...
  it("marks every matching scoped draft when multiple environments share a thread id", () => {
    const store = useComposerDraftStore.getState();
    const localThreadRef = scopeThreadRef(TEST_ENVIRONMENT_ID, threadId);
    const secondaryThreadRef = scopeThreadRef(OTHER_TEST_ENVIRONMENT_ID, threadId);

    store.setProjectDraftThreadId(projectRef, localDraftId, { threadId });
    store.setPrompt(localDraftId, "local draft");
    store.setProjectDraftThreadId(secondaryProjectRef, secondaryDraftId, { threadId });
    store.setPrompt(secondaryDraftId, "secondary draft");

    markPromotedDraftThread(threadId);

    expect(store.getDraftThreadByProjectRef(projectRef)).toBeNull();
    expect(store.getDraftThreadByProjectRef(secondaryProjectRef)).toBeNull();
    expect(store.getDraftThreadByRef(localThreadRef)?.promotedTo).toEqual(localThreadRef);
    expect(store.getDraftThreadByRef(secondaryThreadRef)?.promotedTo).toEqual(secondaryThreadRef);
    expect(draftByKey(localDraftId)?.prompt).toBe("local draft");
    expect(draftByKey(secondaryDraftId)?.prompt).toBe("secondary draft");
  });
...
describe("composerDraftStore modelSelection", () => {
  ...
  it("keeps explicit custom-provider reset overrides on the selection", () => {
    const store = useComposerDraftStore.getState();

    store.setModelSelection(
      threadRef,
      modelSelection(CUSTOM_DRIVER, "claude-opus-4-6", {
        reasoning: "xhigh",
        fastMode: true,
        thinking: false,
      }),
    ...
    expect(
      draftFor(threadId, TEST_ENVIRONMENT_ID)?.modelSelectionByProvider[CUSTOM_INSTANCE],
    ).toEqual(
      modelSelection(CUSTOM_DRIVER, "claude-opus-4-6", {
        reasoning: "medium",
        fastMode: false,
  ...
  it("preserves the selected custom-provider model when only traits change", () => {
    const store = useComposerDraftStore.getState();

    store.setProviderModelOptions(threadRef, CUSTOM_DRIVER, toSelections({ reasoning: "high" }), {
      model: "gpt-5.4",
      persistSticky: true,
    });
    ...
    expect(
      ...
      modelSelection(CUSTOM_DRIVER, "gpt-5.4", {
        reasoning: "high",
      }),
    );
    expect(
      useComposerDraftStore.getState().stickyModelSelectionByProvider[CUSTOM_INSTANCE],
    ).toEqual(
      modelSelection(CUSTOM_DRIVER, "gpt-5.4", {
        reasoning: "high",
      }),
...
describe("composerDraftStore sticky composer settings", () => {
  ...
  it("drops empty custom-provider model options when normalizing sticky state", () => {
    const store = useComposerDraftStore.getState();
    ...
    expect(
      useComposerDraftStore.getState().stickyModelSelectionByProvider[CUSTOM_INSTANCE],
    ).toEqual(modelSelection(CUSTOM_DRIVER, "gpt-5.4"));
    expect(useComposerDraftStore.getState().stickyActiveProvider).toBe("custom");
  });
