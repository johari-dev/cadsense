...
const PrimaryEnvironmentTargetSource = Schema.Literals(["window-origin", "desktop-managed"]);
...
function resolveHttpRequestBaseUrl(primaryTarget: PrimaryEnvironmentTarget): string {
  ...
  const devServerUrlValue = import.meta.env.VITE_DEV_SERVER_URL?.trim();
  if (!devServerUrlValue) {
    return httpBaseUrl;
  }
...
export function readPrimaryEnvironmentTarget(): PrimaryEnvironmentTarget {
  return resolveDesktopPrimaryTarget() ?? resolveWindowOriginPrimaryTarget();
}
