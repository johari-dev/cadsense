...
describe("derivePendingApprovals", () => {
  ...
  it("keeps app access approvals and persistence choices from provider activities", () => {
    const options = [
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "cadsense · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "cadsense · preview_status",
          data: { item },
        },
      }),
    ];
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("cadsense · preview_status");
  ...
  it("keeps MCP payloads while collapsing lifecycle updates", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        kind: "tool.updated",
        summary: "cadsense · preview_snapshot",
        payload: {
          itemType: "mcp_tool_call",
          toolCallId: "call-1",
          data: { item },
...
describe("session activity performance", () => {
  it("reuses entries for unchanged activities", () => {
    const activities = ["test", "lint", "build"].map((command, index) =>
      makeActivity({
        ...
        payload: {
          itemType: "command_execution",
          data: { toolCallId: `stable-tool-${index}`, item: { command: ["vp", "run", command] } },
        },
      }),
    );
