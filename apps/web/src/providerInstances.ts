...
export const NO_PROVIDER_MODEL_SELECTION: ModelSelection = {
  instanceId: ProviderInstanceId.make("cadsense_no_provider"),
  model: "",
};
 ...
 * before any custom instances of the same kind. Codex appears before Claude;
 * unknown driver kinds follow in their original order.
 */
export function sortProviderInstanceEntries(
  entries: ReadonlyArray<ProviderInstanceEntry>,
): ReadonlyArray<ProviderInstanceEntry> {
  // Group by driver kind preserving first-appearance order, then emit the two
  // built-in drivers in product order and any unknown kinds afterward.
  ...
  const preferredKinds = [ProviderDriverKind.make("codex"), ProviderDriverKind.make("claudeAgent")];
  const orderedKinds = [
    ...preferredKinds.filter((kind) => byKind.has(kind)),
    ...Array.from(byKind.keys()).filter((kind) => !preferredKinds.includes(kind)),
  ];
