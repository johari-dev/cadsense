...
import { UnifiedSettings } from "@cadsense/contracts/settings";
const isRuntimeMode = Schema.is(RuntimeMode);
const isProviderDriverKind = Schema.is(ProviderDriverKind);

export const COMPOSER_DRAFT_STORAGE_KEY = "cadsense:composer-drafts:v1";
...
export interface ComposerThreadDraftState {
   ...
   * inline so it can be restored on reload.
   */
   ...
   * built-in `ProviderDriverKind` literals (`codex` / `claudeAgent`) also
   * satisfy the `ProviderInstanceId` slug pattern, so
 ...
 * project and execution settings before the first send.
 */
...
interface ComposerDraftStoreState {
   ...
   * another environment stay in the source draft. Element context and preview
   * annotations also stay in the source.
   */
