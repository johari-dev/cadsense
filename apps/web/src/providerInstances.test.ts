...
describe("sortProviderInstanceEntries", () => {
  it("puts Codex and its default instance before Claude regardless of server order", () => {
...
describe("deriveProviderEntriesByEnvironment", () => {
  it("keeps same-id default instances distinct per environment", () => {
    ...
    expect(byEnvironment.get("secondary")?.get("claude")?.displayName).toBe("Claude Secondary");
    expect(byEnvironment.get("secondary")?.get("claude")?.accentColor).toBe("#445566");
  });
...
describe("resolveDefaultProviderModelSelection", () => {
  it("defaults a new selection to GPT-5.6 Sol even when Claude arrives first", () => {
    const providers = [
      provider({
        provider: ProviderDriverKind.make("claudeAgent"),
        instanceId: "claudeAgent",
      ...
      provider({
        provider: ProviderDriverKind.make("codex"),
        instanceId: "codex",
        models: [model("gpt-5.6-sol", false, true)],
      }),
    ];
