...
const remoteDebuggingPort = process.env.CADSENSE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line cadsense/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.CADSENSE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.CADSENSE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--cadsense-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--cadsense-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
