...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Cadsense (Dev)" : "Cadsense (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.cadsense.cadsense.dev.${devBundleIdSuffix || "local"}`
  : "com.cadsense.cadsense";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["cadsense-dev"] : ["cadsense"];
...
// oxlint-disable-next-line cadsense/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["CADSENSE_PORT", environment.CADSENSE_PORT],
    ["CADSENSE_HOME", environment.CADSENSE_HOME],
    ["CADSENSE_COMMIT_HASH", environment.CADSENSE_COMMIT_HASH],
    ["CADSENSE_OTLP_TRACES_URL", environment.CADSENSE_OTLP_TRACES_URL],
    ["CADSENSE_OTLP_EXPORT_INTERVAL_MS", environment.CADSENSE_OTLP_EXPORT_INTERVAL_MS],
    ["CADSENSE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --cadsense-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Cadsense name.
