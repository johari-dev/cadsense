...
const OVERLAY_ATTRIBUTE = "data-cadsense-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--cadsense-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--cadsense-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--cadsense-radius": theme.radius,
    "--cadsense-background": theme.background,
    "--cadsense-foreground": theme.foreground,
    "--cadsense-popover": theme.popover,
    "--cadsense-popover-foreground": theme.popoverForeground,
    "--cadsense-primary": theme.primary,
    "--cadsense-primary-foreground": theme.primaryForeground,
    "--cadsense-muted": theme.muted,
    "--cadsense-muted-foreground": theme.mutedForeground,
    "--cadsense-accent": theme.accent,
    "--cadsense-accent-foreground": theme.accentForeground,
    "--cadsense-border": theme.border,
    "--cadsense-input": theme.input,
    "--cadsense-ring": theme.ring,
    "--cadsense-font-sans": theme.fontSans,
    "--cadsense-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-cadsense-annotation-tool] body, html[data-cadsense-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-cadsense-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--cadsense-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-cadsense-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
