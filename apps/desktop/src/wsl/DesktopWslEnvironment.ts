...
import { satisfiesSemverRange } from "@cadsense/shared/semver";
...
>()("@cadsense/desktop/wsl/DesktopWslEnvironment") {}
...
const buildWslNodeEngineCheckScript = (): string => `${satisfiesSemverRange.toString()}
(${wslNodeEngineCheckMain.toString()})();`;

const buildWslNodeEnvScript = (nodeEngineRange?: string | null): string => `prepend_path_if_dir() {
  if [ -d "$1" ]; then
    case ":$PATH:" in
      *":$1:"*) ;;
      *) PATH="$1:$PATH" ;;
...
wsl_node_satisfies_engine() {
  CADSENSE_NODE_ENGINE_RANGE=${shellSingleQuote(nodeEngineRange?.trim() || "")}
  if [ -z "$CADSENSE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$CADSENSE_NODE_ENGINE_RANGE" <<'NODE'
${buildWslNodeEngineCheckScript()}
NODE
}
...
ensure_wsl_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for CADSENSE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$CADSENSE_NODE_BIN/node" ]; then
        PATH="$CADSENSE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".cadsense-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".cadsense-wsl-runtime-selected";
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    '    [ -f "${candidate%/*}/cadsense-wsl-node-pty.json" ] || continue',
    "    return 0",
    "  done",
    "  return 1",
    "}",
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.cadsense/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
export const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this cadsense build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
...
//
// The sentinel must be a package the packaged server bundle leaves external. It used to be
...
const marker = path.join(prebuildDir, "cadsense-wsl-node-pty.json");
...
const NODE_PTY_BUILD_SCRIPT = (linuxServerDir: string) =>
  [
    ...
    `printf '{"arch":"%s","modules":"%s","nodePtyVersion":"%s"}\\n' "$arch" "$modules" "$node_pty_version" > "$prebuild_dir/cadsense-wsl-node-pty.json"`,
...
// Pulls the absolute node path after the WSL resolver repaired PATH. Returns
// null when no node was found, which the caller
