...
> =>
  Effect.gen(function* () {
    ...
    const tempDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "cadsense-wsl-server-tree-test-",
    });
    ...
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        const marker = yield* fileSystem.readFileString(
          path.join(root, "cadsense-wsl-server-tree.json"),
        );
  ...
  it.effect("sweeps stale version directories and leftover partials after extraction", () =>
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        // CADSENSE_HOME is set to tempDir, so the desktop state dir resolves to
        // <tempDir>/userdata (no .cadsense segment).
