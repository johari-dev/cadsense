import { fromLenientJson } from "@cadsense/shared/schemaJson";
...
function resolveEarlyDesktopSettingsPath(input: {
  ...
  const cadsenseHome = Option.fromUndefinedOr(input.env.CADSENSE_HOME);
...
export function resolveEarlyLinuxElectronOptions(
  ...
  return {
    linuxWmClass: isDevelopmentEnvironment(input.env) ? "cadsense-dev" : "cadsense",
