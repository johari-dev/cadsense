...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.desktopSettingsPath, "/tmp/cadsense/userdata/desktop-settings.json");
      assert.equal(environment.clientSettingsPath, "/tmp/cadsense/userdata/client-settings.json");
      assert.equal(environment.serverSettingsPath, "/tmp/cadsense/userdata/settings.json");
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.cadsense/dev");
      assert.equal(production.stateDir, "/Users/alice/.cadsense/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          CADSENSE_DESKTOP_APP_USER_MODEL_ID: " com.cadsense.cadsense.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.cadsense.cadsense.dev.local");
    }),
  );
