...
export class DesktopBackendConfiguration extends Context.Service<
  ...
  {
    readonly setPrimaryPort: (port: number) => Effect.Effect<void>;
    readonly primaryPort: Effect.Effect<number>;
    // Build the Windows-native primary backend's loopback-only start config.
    readonly resolvePrimary: Effect.Effect<
      DesktopBackendManager.DesktopBackendStartConfig,
      PlatformError.PlatformError
    >;
    // Build a WSL backend start config for the given distro on the given
    // port. Distro=null means "WSL default distro" and is
...
>()("@cadsense/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = ["CADSENSE_PORT"] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "cadsense-resource-monitor.exe" : "cadsense-resource-monitor";
}
...
interface WslPreflightSuccess {
  ...
  // Absolute path to the node binary the WSL preflight validated after repairing
  // PATH. The launch must use this exact path so it
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's CADSENSE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  // the desktop's readiness probe and renderer connection both saw
  // "Failed to fetch" when the backend only bound to
  // 127.0.0.1 inside WSL. Binding to 0.0.0.0 plus advertising the
  // WSL IP as the renderer-visible URL avoids that dependency. This remains
  // local-machine traffic over the WSL virtual network.
  const wslBindHost = "0.0.0.0";

  const bootstrap = {
    port: input.port,
    // Omit cadsenseHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus CADSENSE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.cadsense via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutCadsenseHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "CADSENSE_HOME") continue;
    parentEnvWithoutCadsenseHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutCadsenseHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutCadsenseHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus CADSENSE_HOME; pass it
...
export const make = Effect.gen(function* () {
  ...
  const primaryPortRef = yield* Ref.make(3773);
  ...
  const buildWslPrimaryConfig = Effect.gen(function* () {
    const primaryPort = yield* Ref.get(primaryPortRef);
  ...
  const buildWindowsPrimaryConfig = Effect.gen(function* () {
    const shared = yield* sharedInputs;
    const port = yield* Ref.get(primaryPortRef);
    const resourceMonitorPath = yield* resolveResourceMonitorPath().pipe(
      Effect.provideService(FileSystem.FileSystem, fileSystem),
      Effect.provideService(DesktopEnvironment.DesktopEnvironment, environment),
    );
    return yield* resolvePrimaryStartConfig({ ...shared, resourceMonitorPath, port }).pipe(
      Effect.provideService(DesktopEnvironment.DesktopEnvironment, environment),
    );
  });
  ...
  return DesktopBackendConfiguration.of({
    setPrimaryPort: (port) => Ref.set(primaryPortRef, port),
    primaryPort: Ref.get(primaryPortRef),
