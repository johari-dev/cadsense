...
import type { DesktopUpdateState } from "@cadsense/contracts";
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    homeDirectory: `/tmp/cadsense-desktop-updates-home-${process.pid}`,
    platform: "darwin",
    processArch: "x64",
    appVersion: "1.2.3",
    appPath: "/repo",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          CADSENSE_HOME: `/tmp/cadsense-desktop-updates-test-${process.pid}`,
          CADSENSE_DESKTOP_MOCK_UPDATES: "true",
          CADSENSE_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        harness.emit("update-available", {
          ...
          releaseNotes: [
            ...
            },
            {
              ...
              note: "- [codex] Improve updater stack by @juliusmarminge in #3821",
            },
          ],
        });
        yield* flushCallbacks;
        ...
        assert.deepEqual(state.releaseNotes, [
          ...
          {
            version: "1.2.4-nightly.20260709.765",
            items: ["[codex] Improve updater stack by @juliusmarminge in #3821"],
          },
        ]);
