...
import { DEFAULT_CLIENT_SETTINGS } from "@cadsense/contracts";
...
// sRGB equivalents of the fixed renderer palette's neutral OKLCH colors.
const WINDOW_BACKGROUND_COLOR = "#141414";
const TITLEBAR_SYMBOL_COLOR = "#d4d4d4";
const SPLASH_LABEL_COLOR = "#a8a8a8";
const SPLASH_ACCENT_COLOR = "#cfcfcf";
const SPLASH_TRACK_COLOR = "rgba(207,207,207,0.18)";
...
>()("@cadsense/desktop/window/DesktopWindow") {}
...
function buildConnectingSplashDataUrl(): string {
  const background = WINDOW_BACKGROUND_COLOR;
  const label = SPLASH_LABEL_COLOR;
  const accent = SPLASH_ACCENT_COLOR;
  const track = SPLASH_TRACK_COLOR;
...
function getWindowTitleBarOptions(platform: NodeJS.Platform): WindowTitleBarOptions {
  if (platform === "darwin") {
    return {
...
export const make = Effect.gen(function* () {
  ...
  > {
    ...
    const window = yield* electronWindow.create({
      ...
      ...getWindowTitleBarOptions(environment.platform),
      webPreferences: {
  ...
  const showConnectingSplash = Effect.gen(function* () {
    ...
    void splash.loadURL(buildConnectingSplashDataUrl());
