...
export const openInFileManager = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.OPEN_IN_FILE_MANAGER_CHANNEL,
  payload: OpenInFileManagerInput,
  result: Schema.Boolean,
  handler: Effect.fn("desktop.ipc.window.openInFileManager")(function* (input) {
    const shell = yield* ElectronShell.ElectronShell;
    return yield* shell.openInFileManager(input);
  }),
});
