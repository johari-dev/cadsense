...
>()("@cadsense/desktop/shell/DesktopShellEnvironment") {}
...
const startMarker = (name: string) => `__CADSENSE_ENV_${name}_START__`;
const endMarker = (name: string) => `__CADSENSE_ENV_${name}_END__`;
  ...
  function* (
    ...
    for (const [index, command] of WINDOWS_SHELL_CANDIDATES.entries()) {
      const output = yield* runCommandOutput({
        ...
        args,
        timeout: LOGIN_SHELL_TIMEOUT,
        // PowerShell 7 is optional. Only report a failure after Windows
        // PowerShell, the final fallback, also fails.
