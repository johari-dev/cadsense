...
function envOutput(values: Readonly<Record<string, string>>): string {
  return Object.entries(values)
    .flatMap(([name, value]) => [
      `__CADSENSE_ENV_${name}_START__`,
      value,
      `__CADSENSE_ENV_${name}_END__`,
    ])
    .join("\n");
}
...
describe("DesktopShellEnvironment", () => {
  ...
  it.effect("silently falls back when optional PowerShell 7 is not installed", () => {
    ...
    return runShellEnvironment({
      ...
      platform: "win32",
      failure: (command) =>
        command._tag === "StandardCommand" && command.command === "pwsh.exe" ? notFound : undefined,
      handler: () => envOutput({ PATH: "C:\\Profile\\Node;C:\\Windows\\System32" }),
    }).pipe(
      Effect.andThen(
        Effect.sync(() => {
          ...
          assert.match(env.PATH ?? "", /^C:\\Profile\\Node/);
        }),
      ),
